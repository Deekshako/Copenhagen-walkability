/*
  ...
  ADAPTED FOR COPENHAGEN BY DEEKSHA KO , CHAITALI AND YICHEN LU 
...
// Set up travel time style with the new color scheme from the image
var ttColorScheme = new ColorSchemeNumeric(
  limits_asc = [0, 5, 10, 15, 20, 25, 30],  // Keeping the original limits
  colors = ["#2D6B3F", "#6FA559", "#AFCD76", "#FBD278", "#F17D68", "#C3443C"],  // Colors from the image
  labels = [
    'Under 5 min', '5 - 10 min', '10 - 15 min', '15 - 20 min', '20 - 25 min',
    '25 - 30 min', 'Over 30 min'  // Keeping the original labels
  ]
);

// Adjust the legend HTML to ensure text color is white
var tt_legend_html = (
  '<p style="text-align:center; margin:0px 0px 4px 0px; line-height:16px; color: white;">' +
  '<strong>Walking time</strong></p>' +
  '<div style="color: white;">' +  // Wrap the legend in a div with white text color
  ttColorScheme.legend_html() +  // Generate the legend HTML
  '</div>'
);
...
function tt_style(feature) {
  return {
    ...
    weight: .25, opacity: 1, color: '#222', fillOpacity: 0.9
  };
}
...
// Create the basemap, centered on Copenhagen
var walkability_map = create_choro_map_base(
  map_div_id = 'walkability',
  ctr = [55.6761, 12.5683],  // Set to Copenhagen's coordinates
  bounds = [[55.61, 12.45], [55.73, 12.65]],  // Approximate bounding box for Copenhagen
  minZoom = 11, maxZoom = 16, startZoom = 12,  // Adjust startZoom if necessary
  attr_addon = ''
);
...
info.onAdd = function(map){
    this._div = L.DomUtil.create('div', 'leafinfo movebox');
    this.update();
    return this._div;
};
info.update = function(feature){
  ...
  if (feature == null || map_terms.length == 0) {
    this._div.innerHTML = '<h4 style="margin:0px; color: #D3D3D3; font-size: 12px;">Select destinations, then<br/>hover over a tract for details</h4>';
  } else {
    this._div.innerHTML = '<h4 style="margin:0px; color: #D3D3D3; font-size: 12px;"><u>Walking time</u></h4>';
    for (d of map_terms) {
      ...
      // Add to inner div
      this._div.innerHTML += (
        '<p style="margin:0px; color: #D3D3D3; font-size: 14px;">' + destination_label + ': ' + tt_label + '</p>'
      );
    }
  }
}; 
