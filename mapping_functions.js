...
function create_choro_map_base(
  ...
  'https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}.png',  // Removed the {r} parameter
  {subdomains: 'abcd', maxZoom: 20, pane: 'tilePane'}
  ).addTo(basemap);

   var Stamen_TonerLines = L.tileLayer(
  'https://stamen-tiles-{s}.a.ssl.fastly.net/toner-lines/{z}/{x}/{y}.png',  // Removed the {r} parameter
  {subdomains: 'abcd', maxZoom: 15, opacity: 0.5, pane: 'markerPane', zIndex: 1}
  ).addTo(basemap);

   var CartoDB_PositronOnlyLabels = L.tileLayer(
  'https://{s}.basemaps.cartocdn.com/dark_only_labels/{z}/{x}/{y}.png',  // Removed the {r} parameter
  {attribution: attr, subdomains: 'abcd', maxZoom: 20, pane: 'markerPane', zIndex: 2}
  ).addTo(basemap);
