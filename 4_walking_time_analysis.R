...
# Use 'id' instead of 'GEOID', and ensure the population column exists
origins <- cbind(
  as.data.table(prepared_blocks)[, .(id, Population)],
...
# Define the types of opportunities
...
for (o_type in opportunity_types) {
  ...
  travel_times_list[[o_type]] <- travel_times_by_od_pair[
    , .(travel_time = min(travel_time_p50, na.rm = TRUE)),
    by = .(id = from_id)
  ][, type := o_type]
}
tictoc::toc()
...
)] |> data.table::dcast.data.table(id ~ type, value.var = 'travel_time')
...
bb_text <- glue::glue("var bounding_box = {jsonlite::toJSON(bb, auto_unbox = TRUE)};")
